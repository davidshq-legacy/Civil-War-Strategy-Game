SUB armies
	...
	mtx$(0) = "To"
	FOR i = 1 TO 6
		IF matrix(target, i) > 0 THEN size = size + 1: mtx$(size) = city$(matrix(target, i)): array(size) = matrix(target, i)
	NEXT i
	CALL bubble(size)
	...
	IF month < 3 AND jancam = 0 AND cityp(array(choose)) <> side THEN 
		COLOR 11: CALL clrbot: PRINT "No campaigns in January"; : CALL TICK(9): CALL clrbot: GOTO dudd
	END IF
